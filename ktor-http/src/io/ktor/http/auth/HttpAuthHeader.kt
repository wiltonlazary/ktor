...
sealed class HttpAuthHeader(val authScheme: String) {
    ...
    class Parameterized(
        ...
        fun withParameter(name: String, value: String) =
            Parameterized(authScheme, this.parameters + HeaderValueParam(name, value))

        override fun render(encoding: HeaderValueEncoding) =
            parameters.joinToString(", ", prefix = "$authScheme ") {
                "${it.name}=${it.value.encode(encoding)}"
            }
...
private fun String.substringAfterMatch(match: MatchResult): String =
    drop(match.range.endInclusive + if (match.range.isEmpty()) 0 else 1)
